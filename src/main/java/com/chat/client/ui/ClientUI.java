 ...
 *   <li>{@code HISTORY_RESULT} -> 私聊窗口会话补拉/向前翻页渲染到窗口，主动查询则弹出对话框；</li>
 *   <li>{@code SEARCH_RESULT} -> 弹出搜索结果对话框，双击命中行可跳转到对应会话；</li>
 *   <li>{@code MSG_RECALL} -> 本人撤回回执或对端撤回通知，替换对应气泡为撤回提示；</li>
 ...
 * <p>数据规模相关入口：</p>
 * <ul>
 *   <li>历史记录按"上一页最小主键"分页拉取，私聊窗口顶部提供"加载更早的消息"，
 *       向前插入后补偿滚动位置，使用者正在看的那一行不会跳动；</li>
 *   <li>关键字搜索在服务端完成（正文以密文入库，只有服务端能解密匹配），
 *       结果以表格对话框展示，双击可跳转；</li>
 *   <li>撤回入口在本窗口菜单中列出"2 分钟内本人发送的消息"，服务端回执后替换对应气泡。</li>
 * </ul>
 *
