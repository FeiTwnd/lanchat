...
会话恢复共用 `SESSION_RESUME` 类型承载两种语义：登录成功后服务端下发令牌（正文即令牌），重连时客户端携带令牌请求恢复，服务端回 `OK` 或 `EXPIRED|原因`；令牌无效时客户端回退为重新登录。令牌只保存在内存中，不落盘、不写日志。这部分可靠性能力已在客户端网络层与服务端落地，界面侧提供连接状态与消息发送状态提示；历史记录分页、消息搜索与消息撤回的界面入口均已接通，已读回执（`read_flag`）仍属后续扩展方向。
...
| 界面增强（私聊与全局未读角标、断线横幅、头像资料卡） | 集中在 `com.chat.client.ui` 与 `com.chat.client.ui.theme`，消息协议与服务端无需改动 | 界面轮次剩余范围；消息气泡、多行输入、智能时间戳、空状态、拖拽与剪贴板、表情面板、`@` 提醒、引用回复已在第 3 轮交付 |
