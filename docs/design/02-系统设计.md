...
| 客户端界面模块 | 登录、主窗口、私聊、群聊、文件传输、富文本渲染基类 | `BaseUI`、`BaseChatPanel`、`LoginUI`、`ClientUI`、`PrivateChatUI`、`GroupChatUI`、`FileTransferUI` | `ChatClient` |
...
说明：`PrivateChatPanel` 与 `GroupChatPanel` 是各自窗口内部的包级私有面板，继承 `BaseChatPanel`，负责富文本渲染与输入发送，因此图中仅在 `BaseChatPanel` 处标注其子类关系，未展开全部成员。
