...
chat_message 的 message_id 与 delivered 已接入可靠投递：message_id 是跨端一致的稳定标识（发送方生成 UUID），服务端据此做幂等去重，delivered 标记消息是否已投递给接收方，为 0 的记录在接收方下次登录时补投。read_flag 是为「已读回执」预留的列（尚未接线）；recalled 已用于消息撤回——撤回后读出时以占位文本替换正文，历史查询、关键字检索与导出都不会再泄露原文。message_id 上的唯一索引允许存在多行 NULL，因此升级前写入的历史记录不会互相冲突，无需数据迁移。
...
4. 历史记录分页已接入：协议支持「上一页最小主键 + 每页条数」，服务端改用键集分页查询，私聊窗口提供「加载更早的消息」入口。仍未做的是命中结果的高亮与跳转定位。
5. 消息搜索已实现：服务端按登录态用户名过滤可见范围后逐行解密匹配，客户端提供搜索对话框；尚未做命中上下文的高亮展示。
6. 消息撤回已实现：仅发送者本人、2 分钟时限、重复撤回幂等；撤回后正文以占位文本替换，历史查询、关键字检索与导出三条路径同时不可见原文。已读回执仍是预留能力，read_flag 列未接线。
7. 第 3 轮界面增强已交付：消息气泡（左右分布、系统消息居中、长文本自动换行）、多行输入（Enter 发送 / Shift+Enter 换行、输入法组合期间不误发）、智能时间戳、空状态、拖拽发送文件、剪贴板图片发送、Unicode 表情面板、群聊 @ 提醒与候选补全、引用回复（私聊与群聊）、消息搜索对话框、主窗口菜单中的撤回入口与历史分页入口（撤回入口尚未放到气泡右键菜单）；尚未落地私聊与全局未读角标、断线横幅与头像资料卡。本项另经离屏渲染独立验证（16 项断言全部通过）。改进方向是按「先补齐消息状态、再做交互增强」的顺序分批落地，避免一次性改动过大。
