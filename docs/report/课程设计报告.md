...
| com.chat.server | 8 | 2270 | 服务器单例、连接处理线程、消息分派模板、在线用户管理、UDP 发现应答、服务器观察者与界面 |
| com.chat.client | 4 | 1440 | 客户端网络门面、回调接口、UDP 发现客户端、命令行入口 |
| com.chat.client.ui | 10 | 3065 | 界面基类、聊天面板基类、登录界面、主界面、好友树（OnlineUserTree）、私聊界面、群聊界面、文件传输界面 |
| com.chat.client.ui.theme | 6 | 1285 | 界面皮肤：配色与字体、Java2D 图标与头像（含树形三角）、自绘按钮与标题栏、窗口缩放手柄 |
| 合计 | 55 | 13186 | 主源码（含规范中文注释） |
...
界面层另有两层验证。上一轮界面皮肤化改造完成后执行过一轮完整的窗口级走查，用 Xvfb 虚拟显示加 java.awt.Robot 驱动真实窗口完成 14 项检查并全部通过（详见《测试报告》第十二节）。本轮「精简工程」改造删除了构建脚本、去掉了服务器的控制台启动模式、更换了默认头像、修复了好友树选中态并改变了文件发送入口，但没有重跑那一轮完整走查，而是改用更轻、更稳定的组件级离屏渲染校验：把好友树、头像条、私聊面板与群聊面板离屏渲染为 PNG，再做像素断言与源码核对，共 6 项（UI-01 至 UI-06）且全部通过——好友树分组与人数正确、默认头像为平面绘制且离线明显浅于在线、选中行不含系统外观橙色 #E95420 且含主题浅蓝 #CFE9FA 超过 2000 像素、私聊与群聊窗口都存在「发送文件」按钮、主窗口不再有按用户发送文件的入口。这类校验的定位是「视觉回归的机器可判定部分」，它不能替代完整走查，但足以覆盖本轮改动的界面部分，明细见《测试报告》第十二节与《测试用例表》第五节。
...
2. 主源码：55 个 Java 文件、13186 行（含规范中文注释），按九个包分层组织，覆盖公共模型、持久化、业务、服务器、客户端网络与 Swing 界面；界面部分采用无边框自绘窗口、好友树与 Java2D 自绘皮肤，默认头像为平面人形剪影，不使用任何图片资源。
3. 核心功能：局域网自动发现、注册与登录、在线列表实时刷新、私聊、群聊、文件分块传输与完整性校验、历史查询与导出、昵称与密码修改、管理员删除用户、服务器控制台，共 17 项功能需求全部落地。
4. 测试代码：9 个文件、2284 行，其中 47 个单元测试用例与 7 个集成测试用例全部通过，集成测试真实启动服务器与多个客户端。
5. 交付配套：配置文件 config/chat.properties、数据库脚本 sql/schema.sql、IntelliJ IDEA 运行配置 .idea/runConfigurations/、README、测试报告与测试用例表、用户手册、课程设计报告与答辩 PPT 大纲。
6. 工程细节：常量化、配置外部化、密码加盐散列、身份防伪造、文件完整性校验与残缺文件清理、原子写入、EDT 线程安全、异常与日志体系，以及 18 个真实缺陷的定位与修复记录（D1 至 D7 见本报告 5.6 节，D-08 至 D-18 见《测试报告》第九节）。
...
| 28 | com/chat/server/ChatServer.java | 499 | 服务器单例，端口绑定、线程池、心跳扫描与启动回滚 |
...
| 39 | com/chat/client/ChatClientApp.java | 111 | 客户端命令行入口与日志配置 |
| 40 | com/chat/client/ui/BaseUI.java | 285 | 界面基类，外观、字体、居中、提示框与 EDT 调度 |
| 41 | com/chat/client/ui/BaseChatPanel.java | 317 | 聊天面板基类，富文本显示、输入发送与发送文件入口 |
| 42 | com/chat/client/ui/LoginUI.java | 652 | 登录、注册、发现服务器与修改密码界面 |
| 43 | com/chat/client/ui/ClientUI.java | 716 | 主界面，好友树、功能按钮与消息分派 |
| 44 | com/chat/client/ui/PrivateChatUI.java | 62 | 私聊窗口，负责窗口装配与标题 |
| 45 | com/chat/client/ui/PrivateChatPanel.java | 121 | 私聊面板，覆写过滤条件、发送目标与文件接收方 |
| 46 | com/chat/client/ui/GroupChatUI.java | 56 | 群聊窗口，负责窗口装配、注入在线用户名单 |
| 47 | com/chat/client/ui/GroupChatPanel.java | 142 | 群聊面板，接收群聊消息与系统通知，支持群发文件 |
| 48 | com/chat/client/ui/FileTransferUI.java | 373 | 文件传输界面，进度条、传输日志与立即发送入口 |
| 49 | com/chat/client/ui/OnlineUserTree.java | 341 | 好友树，分组、过滤、离线置灰与选中态配色 |
| 50 | com/chat/client/ui/theme/Theme.java | 246 | 配色、字体探测、全局外观与选中色 |
| 51 | com/chat/client/ui/theme/Glyphs.java | 308 | Java2D 绘制的界面图标与树形三角 |
| 52 | com/chat/client/ui/theme/AvatarFactory.java | 164 | 默认头像绘制与缓存 |
...
## A.2 测试代码（9 个文件，2284 行；其中自定义注解定义 Test.java 为 30 行）
...
| 8 | com/chat/test/IntegrationTest.java | 592 | 端到端集成测试 7 个用例 |
| 9 | com/chat/test/JarPackager.java | 144 | 把编译产物打成可执行 jar 的打包辅助类 |
| — | 合计 | 2284 | 9 个测试相关文件 |
