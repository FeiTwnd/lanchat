...
7. 第 3 轮界面增强已交付：消息气泡（左右分布、系统消息居中、长文本自动换行）、多行输入（Enter 发送 / Shift+Enter 换行、输入法组合期间不误发）、智能时间戳、空状态、拖拽发送文件、剪贴板图片发送、Unicode 表情面板、群聊 @ 提醒与候选补全、引用回复（私聊与群聊）、消息搜索对话框、气泡右键撤回与历史分页入口；尚未落地私聊与全局未读角标、断线横幅与头像资料卡。本项另经离屏渲染独立验证（16 项断言全部通过）。改进方向是按「先补齐消息状态、再做交互增强」的顺序分批落地，避免一次性改动过大。
